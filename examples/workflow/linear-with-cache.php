...
$executors = [
    ...
    'done' => new FiberExecutor(
        static fn (WorkflowState $state, string $place): WorkflowState => $state->set('completed_at', date('c')),
    ),
];

// Run the workflow
$agentWorkflow = new AgentWorkflow($workflow, $executors, new WorkflowStateStore(new ArrayAdapter()));
$initialState = new WorkflowState('linear-'.bin2hex(random_bytes(4)), ['topic' => 'The future of space exploration']);
