...
$executors = [
    ...
    // The review step uses CallableExecutor for conditional branching
    'review' => new CallableExecutor(static function (WorkflowStateInterface $state, string $place) use ($reviewerAgent): WorkflowStateInterface {
        ...
        $state = $state->set('review_feedback', $feedback);
    ...
    'approved' => new CallableExecutor(static function (WorkflowStateInterface $state, string $place): WorkflowStateInterface {
        return $state->set('status', 'published');
    }),

    'rejected' => new CallableExecutor(static function (WorkflowStateInterface $state, string $place): WorkflowStateInterface {
