...
/** @var StreamResult|null $secondAnswer */
$secondAnswer = null;
$firstChunks = 0;
foreach ($firstAnswer->getContent() as $delta) {
    ...
    // SpeechSession::call() flips the previous InterruptionSignal (observed by the
    // InterruptionStreamListener attached by SpeechAgent) AND cancels the previous
    // StreamResult, so the first answer stops streaming on the next delta.
    if (3 === $firstChunks) {
        $next = $session->call(new MessageBag(
            Message::ofUser('Actually, just tell me the year Symfony was first released.'),
        ));
        if ($next instanceof StreamResult) {
            $secondAnswer = $next;
        }
        break;
    }
...
if ($secondAnswer instanceof StreamResult) {
