...
$platform = Factory::createPlatform(env('ANTHROPIC_API_KEY'), http_client());
...
$handle = $platform->invoke('claude-sonnet-4-5-20250929', $inputs, [
    'batch' => true,
    'max_tokens' => 50,
])->asJob();
...
// Anthropic takes up to 24 hours, so a real application stores the handle and picks it up in a worker.
$jobClient = Factory::createJobClient(env('ANTHROPIC_API_KEY'), http_client());
...
// The batch ended, so nothing is left to wait for - and a canceled or expired one still hands out
// what it got through.
