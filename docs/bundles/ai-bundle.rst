...
it with an ``mcp_server`` tool entry, so both bundles share one connection instead of opening a
...
An ``mcp_server`` entry sits in the tool list next to ``service`` and ``agent`` entries and
references one connection as ``<client>.<server>``. The server's tools are discovered at runtime and
reach the model prefixed with the server name - a ``read_file`` tool of the ``filesystem`` server
becomes ``filesystem_read_file`` - so several servers can serve one agent without their tool names
colliding. Add ``prefix`` to choose it yourself:
    ...
    ai:
        agent:
            my_agent:
                tools:
                    - 'Symfony\AI\Agent\Bridge\Wikipedia\Wikipedia'
                    - { mcp_server: 'research.filesystem', prefix: 'fs__' }

MCP tools and local tools compose, but they do not share a toolbox: each server becomes a toolbox of
its own, offered to the agent next to the local one through a
:class:`Symfony\\AI\\Agent\\Toolbox\\ChainToolbox`. Listing an ``mcp_server`` therefore does not turn the
local toolbox into an explicit list - combined with ``tools: true``, the agent gets every
``ai.tool``-tagged service *and* the remote ones.
