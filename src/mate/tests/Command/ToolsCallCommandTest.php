...
final class ToolsCallCommandTest extends TestCase
{
    public function testExecuteCallsToolSuccessfully()
    {
        ...
        $this->assertStringContainsString('Extension: _custom', $output);
    ...
    public function testExecuteWithValidateOnlyMode()
    {
        $container = $this->createContainer();
        ...
        $this->assertSame(Command::SUCCESS, $tester->getStatusCode());
        $output = $tester->getDisplay();
        $this->assertStringContainsString('Validation successful', $output);
