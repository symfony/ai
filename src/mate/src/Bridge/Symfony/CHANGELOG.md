 ...
 * Add `symfony-profiler-compare` tool that diffs the collector summary of two profiler profiles (or two comma-separated sets of profiles, averaged, to smooth out run-to-run noise): a numeric delta for every shared numeric field, the raw before/after pair for a shared field that changed but isn't numeric, and an `improved`/`unchanged`/`regressed` verdict from a per-collector leading field, never a guess at one when a collector has no field anyone can call unambiguously better when lower
