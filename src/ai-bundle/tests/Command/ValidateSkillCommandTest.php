...
final class ValidateSkillCommandTest extends TestCase
{
    ...
    public function testExecuteWithInvalidSkill()
    {
        ...
        $metadata2 = new SkillMetadata('invalid-skill', 'Another skill for testing purposes');
        $skill2 = new Skill('Body content.', $metadata2);

        $validationResult = new SkillValidationResult($skill2, ['Field "name" must be kebab-case (e.g. "my-skill"), got "invalid-skill".']);

        $loader = $this->createMock(SkillLoaderInterface::class);
        $loader->method('loadSkills')
            ->willReturn([
                'valid-skill' => $skill1,
                'invalid-skill' => $skill2,
            ]);

        $validator = $this->createMock(SkillValidatorInterface::class);
        $validator->method('validate')
            ->willReturnCallback(static function (SkillInterface $skill) use ($skill1, $skill2, $validationResult): SkillValidationResult {
                if ($skill === $skill2) {
                    return $validationResult;
                }

                return new SkillValidationResult($skill1);
            });
        ...
        $this->assertStringContainsString('invalid-skill', $commandTester->getDisplay());
    ...
    public function testDisplaysSummary()
    {
        ...
        $validationResult = new SkillValidationResult($skill2, ['Field "name" must be kebab-case.']);
