...
return static function (DefinitionConfigurator $configurator): void {
    $configurator->rootNode()
        ->children()
            ...
            ->end()
            ->arrayNode('mcp')
                ->info('MCP (Model Context Protocol) server connections')
                ->useAttributeAsKey('name')
                ->arrayPrototype()
                    ->children()
                        ->enumNode('transport')
                            ->info('Transport type: http, sse, or stdio')
                            ->values(['http', 'sse', 'stdio'])
                            ->isRequired()
                        ->end()
                        ->stringNode('url')
                            ->info('URL for HTTP/SSE transport')
                        ->end()
                        ->stringNode('command')
                            ->info('Command for stdio transport (e.g., "npx", "php", "python")')
                        ->end()
                        ->arrayNode('args')
                            ->info('Arguments for stdio command')
                            ->scalarPrototype()->end()
                        ->end()
                        ->arrayNode('env')
                            ->info('Environment variables for stdio transport')
                            ->scalarPrototype()->end()
                        ->end()
                        ->arrayNode('headers')
                            ->info('HTTP headers for HTTP/SSE transport')
                            ->scalarPrototype()->end()
                        ->end()
                        ->stringNode('http_client')
                            ->defaultValue('http_client')
                            ->info('Service ID of the HTTP client to use (for HTTP/SSE)')
                        ->end()
                        ->integerNode('timeout')
                            ->defaultValue(30)
                            ->info('Timeout in seconds (for stdio)')
                        ->end()
                        ->arrayNode('tools')
                            ->info('List of tool names to expose (empty = all tools)')
                            ->scalarPrototype()->end()
                        ->end()
                    ->end()
                    ->validate()
                        ->ifTrue(static fn (array $v): bool => \in_array($v['transport'], ['http', 'sse'], true) && empty($v['url']))
                        ->thenInvalid('URL is required for HTTP and SSE transports.')
                    ->end()
                    ->validate()
                        ->ifTrue(static fn (array $v): bool => 'stdio' === $v['transport'] && empty($v['command']))
                        ->thenInvalid('Command is required for stdio transport.')
                    ->end()
                ->end()
            ->end()
        ->end()
