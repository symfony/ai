            ...
            ->arrayNode('hybrid')
                ...
                ->children()
                    ...
                    ->booleanNode('normalize_scores')
                        ...
                        ->defaultTrue()
                    ->end()
                    ->floatNode('fuzzy_threshold')
                        ->info('Minimum word_similarity threshold for fuzzy matching (pg_trgm). Default: 0.2')
                        ->defaultValue(0.2)
                        ->min(0.0)
                        ->max(1.0)
                    ->end()
                    ->floatNode('fuzzy_weight')
                        ->info('Weight of fuzzy matching in hybrid search. 0.0 = disabled (default), 1.0 = fuzzy only')
                        ->defaultValue(0.0)
                        ->min(0.0)
                        ->max(1.0)
                    ->end()
