...
final class SetupStoreCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->setHelp(<<<EOF
The <info>%command.name%</info> command set up the message store:
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            throw new RuntimeException(\sprintf('An error occurred while setting up the "%s" message store: ', $storeName).$e->getMessage(), previous: $e);
        }
