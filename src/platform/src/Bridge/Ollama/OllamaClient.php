...
final class OllamaClient implements ModelClientInterface
{
    ...
    public function request(Model $model, array|string $payload, array $options = []): RawHttpResult
    {
        ...
        return match (true) {
            ...
            $model->supports(Capability::INPUT_MESSAGES) && $payload->supportCompletion() => $this->doCompletionRequest($model, $payload, $options),
     ...
     * @param array<string, mixed> $options
     */
    private function doCompletionRequest(Model $model, OllamaPayload $payload, array $options = []): RawHttpResult
    {
        // Revert Ollama's default streaming behavior
        $options['stream'] ??= false;
