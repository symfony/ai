...
final class SpeechResultConverterTest extends TestCase
{
    ...
    {
        // 422 is not mapped by HttpStatusErrorHandlingTrait::throwOnHttpError(), so it falls through
        // to the converter's own generic error message (unlike 5xx, which now yields a ServerException).
        $response = $this->createStub(ResponseInterface::class);
