...
/**
 * Decart image/video editing contract handler.
 *
 * Same URL as {@see GenerateHandler} but takes a file stream alongside the
 * prompt. Distinct contract identifier so the catalog can pick it for
 * image-to-* and video-to-* models.
 *
 * @author Christopher Hertel <mail@christopher-hertel.de>
 */
final class EditClient implements EndpointClientInterface
{
    public const ENDPOINT = 'decart.edit';
    ...
    {
        $sourcePath = $payload['input_image']['path'] ?? $payload['input_video']['path'];

        $envelope = new RequestEnvelope(
            payload: [
                'prompt' => $options['prompt'],
                'data' => fopen($sourcePath, 'r'),
                ...$options,
            ],
