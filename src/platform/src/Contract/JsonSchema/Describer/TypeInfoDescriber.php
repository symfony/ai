    ...
    private function getTypeSchema(Type $type): array
    {
        ...
        switch (true) {
            ...
            case $type->isIdentifiedBy(TypeIdentifier::OBJECT):
                ...
                $discriminatorMapping = $this->classMetadataFactory->getMetadataFor($type->getClassName())->getClassDiscriminatorMapping()?->getTypesMapping();
                if ($discriminatorMapping) {
                    $discriminators = [];
                    foreach ($discriminatorMapping as $discriminator) {
                        $discriminators[] = $this->factory->buildProperties($discriminator);
                    }
