 ...
 * Add asynchronous batch submission: `Platform::invoke($model, $inputs, ['batch' => true])` (with `$inputs` a list of `Batch\BatchInput`) returns a `DeferredResult` whose `asBatchJob()` yields a persistable `Batch\BatchJob` value object; eligible models expose the new `Capability::BATCH`. Supported by the OpenAI and Anthropic bridges.
 * Add `Batch\BatchManager` to poll a submitted batch and fetch its results (`refresh()`, `canFetchResults()`, `fetchResults()`, `cancel()`) from a persisted `Batch\BatchJob`, including partial recovery for cancelled or expired batches. Supported by the OpenAI and Anthropic bridges.
