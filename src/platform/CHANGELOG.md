 ...
 * Add `Mock\Http\CassetteHttpClient` and `Mock\Http\HttpCassette` to record real HTTP responses (when the cassette file is missing) and replay them offline through the real bridge pipeline (Contract, `ModelClient`, `ResultConverter`) in tests
