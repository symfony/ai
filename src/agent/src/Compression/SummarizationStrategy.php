...
final class SummarizationStrategy implements CompressionStrategyInterface
{
    ...
    /**
     * @param string $summaryModel Model to use for summarization (can be a smaller/faster model)
    ...
    public function compress(MessageBag $messages): MessageBag
    {
        ...
        $summary = $this->generateSummary($conversationText);
    ...
    {
        $summaryMessages = new MessageBag(
            Message::ofUser(\sprintf(self::SUMMARIZATION_PROMPT, $conversationText))
        );

        return $this->platform->invoke($this->summaryModel, $summaryMessages)->asText();
    }
}
