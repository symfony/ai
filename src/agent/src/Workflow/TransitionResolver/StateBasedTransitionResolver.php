...
/**
 * Resolves the next transition from the state's next transition hint.
 *
 * If no hint is set, picks the single enabled transition. If multiple
 * transitions are enabled and no hint is set, throws.
 *
...
final class StateBasedTransitionResolver implements TransitionResolverInterface
{
    public function resolve(WorkflowStateInterface $state, string $currentPlace, WorkflowInterface $workflow, object $subject): ?string
    {
        ...
        throw new TransitionResolutionException(\sprintf('Multiple transitions are enabled from place "%s" and no next transition hint was set on the state; set one with WorkflowState::withNextTransition(). Available transitions: "%s".', $currentPlace, implode('", "', $availableNames)));
    }
}
