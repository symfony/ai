...
use Symfony\AI\Agent\Toolbox\Confirmation\RememberablePolicyInterface;
...
final class ConfirmationSubscriberTest extends TestCase
{
    ...
    public function testRememberChoiceWorksWithCustomRememberablePolicy()
    {
        $remembered = [];
        $policy = new class($remembered) implements RememberablePolicyInterface {
            /** @param array<string, PolicyDecision> $remembered */
            ...
            public function decide(ToolCall $toolCall): PolicyDecision
            {
                return $this->remembered[$toolCall->getName()] ?? PolicyDecision::AskUser;
            }

            public function remember(string $toolName, PolicyDecision $decision): void
            {
                $this->remembered[$toolName] = $decision;
            }
        };
    ...
    public function testRememberIsNotCalledOnNonRememberablePolicy()
    {
        $askCount = 0;
        $policy = new class($askCount) implements PolicyInterface {
            public function __construct(private int &$askCount)
            {
            }
        ...
        $handler = new class implements ConfirmationHandlerInterface {
            public function requestConfirmation(ToolCall $toolCall): ConfirmationResult
            {
                return ConfirmationResult::always(); // wants to remember, but policy can't
            }
        };

        $subscriber = new ConfirmationSubscriber($policy, $handler);

        $subscriber->onToolCallRequested($this->createEvent('tool_a'));
        $subscriber->onToolCallRequested($this->createEvent('tool_a'));

        // Policy was asked twice because it doesn't implement RememberablePolicyInterface
        $this->assertSame(2, $askCount);
    }
