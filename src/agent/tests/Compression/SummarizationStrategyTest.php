...
final class SummarizationStrategyTest extends TestCase
{
    ...
    public function testCompressSummarizesOldMessagesAndKeepsRecent()
    {
        ...
        $this->assertInstanceOf(\Symfony\AI\Platform\Message\UserMessage::class, $resultMessages[1]);
    ...
    {
        $capturedInput = null;
        $platform = new InMemoryPlatform(static function ($model, $input) use (&$capturedInput) {
            $capturedInput = $input;
        ...
        $promptContent = $capturedInput->getMessages()[0]->asText();
        $this->assertStringContainsString('User: Hello, how are you?', $promptContent);
    }
    ...
    public function testCompressFormatsAssistantMessages()
    {
        ...
        $this->assertNotNull($capturedInput);
        $promptContent = $capturedInput->getMessages()[0]->asText();
        $this->assertStringContainsString('Assistant: I am doing well, thank you!', $promptContent);
    }
    ...
    public function testCompressFormatsToolCallMessages()
    {
        ...
        $this->assertNotNull($capturedInput);
        $promptContent = $capturedInput->getMessages()[0]->asText();
        $this->assertStringContainsString('Tool: [search_tool] Search results: found 5 items', $promptContent);
    }
    ...
    public function testCompressTruncatesLongToolCallResults()
    {
        ...
        $this->assertNotNull($capturedInput);
        $promptContent = $capturedInput->getMessages()[0]->asText();
        $this->assertStringContainsString('[long_tool] '.str_repeat('a', 200).'...', $promptContent);
    }
    ...
    public function testCompressSkipsEmptyMessages()
    {
        ...
        $this->assertNotNull($capturedInput);
        $promptContent = $capturedInput->getMessages()[0]->asText();
        $this->assertStringContainsString('User: Valid message', $promptContent);
        $this->assertStringNotContainsString('Assistant:', $promptContent);
    }
