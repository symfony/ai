...
final class SpeechSessionTest extends TestCase
{
    ...
    public function testNewCallAbortsInFlightStreamObservingThePreviousSignal()
    {
        $firstStream = new StreamResult((static function () {
        ...
        // The session injects a fresh InterruptionSignal into options at every call();
        // SpeechAgent attaches an InterruptionStreamListener watching that signal.
        // Here we simulate the same wiring on the first call so the flow is end-to-end.
        $firstSignal = null;
        $innerAgent = $this->createMock(AgentInterface::class);
        $innerAgent->expects($this->exactly(2))
            ->method('call')
            ->willReturnCallback(static function (MessageBag $messages, array $options) use (&$firstSignal, $firstStream): StreamResult|TextResult {
                if (null === $firstSignal) {
                    $firstSignal = $options['interruption_signal'];
                    \assert($firstSignal instanceof InterruptionSignalInterface);
                    $firstStream->addListener(new InterruptionStreamListener($firstSignal));

                    return $firstStream;
                }

                return new TextResult('superseded');
            });

        $session = new SpeechSession($innerAgent);
        $first = $session->call(new MessageBag(Message::ofUser('first')));

        $this->assertSame($firstStream, $first);

        $observed = [];

        foreach ($first->getContent() as $delta) {
            $this->assertInstanceOf(TextDelta::class, $delta);
            $observed[] = $delta->getText();

            if ('chunk-2' === $delta->getText()) {
                // A fresh user input arrives mid-stream: the session flips the
                // previous signal AND cancels the previous stream, so iteration
                // exits cleanly at the next boundary without an exception.
                $session->call(new MessageBag(Message::ofUser('second')));
            }
        }

        $this->assertSame(['chunk-1', 'chunk-2'], $observed, 'Iteration must stop right after the chunk that triggered the second call.');
        $this->assertTrue($firstSignal->isInterrupted(), 'The previous signal must be flipped by SpeechSession::supersede().');
        $this->assertTrue($firstStream->isCancelled(), 'The previous stream must be cancelled by SpeechSession::supersede().');
    }
