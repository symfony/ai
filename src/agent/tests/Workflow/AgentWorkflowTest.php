...
final class AgentWorkflowTest extends TestCase
{
    public function testRunLinearWorkflow()
    {
        ...
        $executors = [
            'start' => $this->createExecutor(static fn (WorkflowState $state): WorkflowState => $state->set('step1', 'done')),
            'middle' => $this->createExecutor(static fn (WorkflowState $state): WorkflowState => $state->set('step2', 'done')),
            'end' => $this->createExecutor(static fn (WorkflowState $state): WorkflowState => $state->set('step3', 'done')),
        ];
    ...
    public function testRunWithConditionalBranching()
    {
        ...
        $executors = [
            'draft' => $this->createExecutor(static function (WorkflowState $state): WorkflowState {
                return $state->set('draft_text', 'Hello');
            }),
            'review' => new FiberExecutor(static function (WorkflowState $state, string $place): WorkflowState {
                return $state->withNextTransition('approve')->set('reviewed', true);
            }),
            'approved' => $this->createExecutor(static function (WorkflowState $state): WorkflowState {
                return $state->set('status', 'approved');
            }),
            'rejected' => $this->createExecutor(static function (WorkflowState $state): WorkflowState {
    ...
    public function testRunPersistsStateAfterEachPlace()
    {
        ...
        $executors = [
            'start' => $this->createExecutor(static function (WorkflowState $state): WorkflowState {
                return $state->set('a', 1);
            }),
            'middle' => $this->createExecutor(static function (WorkflowState $state): WorkflowState {
                return $state->set('b', 2);
            }),
            'end' => $this->createExecutor(static function (WorkflowState $state): WorkflowState {
    ...
    public function testResumeFromPersistedState()
    {
        ...
        $callCount = 0;
        $executors = [
            'step1' => $this->createExecutor(static function (WorkflowState $state) use (&$callCount): WorkflowState {
                ++$callCount;

                return $state->set('step1_result', 'done');
            }),
            'step2' => $this->createExecutor(static function (WorkflowState $state): WorkflowState {
                return $state->set('step2_result', 'done');
            }),
            'step3' => $this->createExecutor(static function (WorkflowState $state): WorkflowState {
    ...
    public function testGuardOnSpecificPlaceOnly()
    {
        ...
        $executorCallLog = [];
        $executors = [
            'start' => $this->createExecutor(static function (WorkflowState $state) use (&$executorCallLog): WorkflowState {
                $executorCallLog[] = 'start';

                return $state->set('step1', 'done');
            }),
            'middle' => $this->createExecutor(static function (WorkflowState $state) use (&$executorCallLog): WorkflowState {
                $executorCallLog[] = 'middle';

                return $state->set('step2', 'done');
            }),
            'end' => $this->createExecutor(static function (WorkflowState $state) use (&$executorCallLog): WorkflowState {
                $executorCallLog[] = 'end';
    ...
    private function createExecutor(\Closure $callback): ExecutorInterface
    {
        return new class($callback) implements ExecutorInterface {
            ...
            public function execute(WorkflowStateInterface $state, string $place): WorkflowState
            {
