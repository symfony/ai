...
final class WorkflowStateStoreTest extends TestCase
{
    ...
    public function testDropScansAndDeletesPrefixedKeys()
    {
        $redis = $this->createMock(\Redis::class);
        $redis->method('scan')->willReturn(['_workflow_state:a', '_workflow_state:b']);
        $redis->expects($this->once())
            ->method('del')
            ->with(['_workflow_state:a', '_workflow_state:b']);
    ...
    {
        $redis = $this->createMock(\Redis::class);
        $redis->method('scan')->willReturn(false);
    ...
    {
        $redis = $this->createMock(\Redis::class);
        $redis->method('scan')->willReturn(['_workflow_state:id-a', '_workflow_state:id-b']);
    ...
    {
        $redis = $this->createMock(\Redis::class);
        $redis->method('scan')->willReturn(['_workflow_state:id-b']);
