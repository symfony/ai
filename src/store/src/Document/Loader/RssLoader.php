 ...
 */
final readonly class RssLoader implements LoaderInterface
{
    ...
    public function load(?string $source, array $options = []): iterable
    {
        ...
        try {
            $xml = $this->httpClient->request('GET', $source)->getContent();
        } catch (ExceptionInterface $exception) {
            throw new RuntimeException(\sprintf('Failed to fetch RSS feed "%s": %s', $source, $exception->getMessage()), previous: $exception);
        }
