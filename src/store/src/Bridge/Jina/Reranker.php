...
namespace Symfony\AI\Store\Bridge\Jina;
...
{
    private const API_URL = 'https://api.jina.ai/v1/rerank';

    public function __construct(
        private readonly HttpClientInterface $client,
        private readonly string $apiKey,
        private readonly string $model = 'jina-reranker-v2-base-multilingual',
    ) {
    }
    ...
    public function rerank(string $query, array $documents, int $topK = 5): array
    {
        ...
        $texts = array_map(
            static fn (VectorDocument $doc): array => ['text' => $doc->getMetadata()->getText() ?? $doc->getMetadata()->getSource() ?? ''],
            $documents,
        );
