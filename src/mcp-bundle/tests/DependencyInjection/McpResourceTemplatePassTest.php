...
#[CoversClass(McpResourceTemplatePass::class)]
class McpResourceTemplatePassTest extends TestCase
{
    public function testCreatesServiceLocatorForTaggedServices()
    {
        ...
        $container->register('template1', 'MockResourceTemplate1')
            ->addTag('mcp.resource_template');

        $container->register('template2', 'MockResourceTemplate2')
            ->addTag('mcp.resource_template');

        $container->register('not_a_template', 'MockService');

        $pass = new McpResourceTemplatePass();
    ...
    public function testDoesNothingWhenNoResourceTemplatesTagged()
    {
        $container = new ContainerBuilder();
        ...
        // Check that server builder doesn't get a container when no templates
        $serverBuilderDefinition = $container->getDefinition('mcp.server.builder');
        $methodCalls = $serverBuilderDefinition->getMethodCalls();
