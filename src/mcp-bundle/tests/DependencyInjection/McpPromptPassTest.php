...
#[CoversClass(McpPromptPass::class)]
class McpPromptPassTest extends TestCase
{
    public function testCreatesServiceLocatorForTaggedServices()
    {
        ...
        $container->register('prompt1', 'MockPrompt1')
            ->addTag('mcp.prompt');

        $container->register('prompt2', 'MockPrompt2')
            ->addTag('mcp.prompt');

        $container->register('not_a_prompt', 'MockService');

        $pass = new McpPromptPass();
    ...
    public function testDoesNothingWhenNoPromptsTagged()
    {
        $container = new ContainerBuilder();
        ...
        // Check that server builder doesn't get a container when no prompts
        $serverBuilderDefinition = $container->getDefinition('mcp.server.builder');
        $methodCalls = $serverBuilderDefinition->getMethodCalls();
