            ...
            ->arrayNode('servers')
                ...
                ->arrayPrototype()
                    ->children()
                        ...
                        ->enumNode('lifecycle')
                            ->values(['handshake', 'stateless'])
                            ->defaultValue('handshake')
                            ->info('"handshake" is the 2025-11-25-and-earlier lifecycle (initialize, sessions, StreamableHttpTransport). "stateless" is 2026-07-28: no handshake, no session, every request self-describing.')
                        ->end()
                        ->arrayNode('protocol_versions')
                            ->info('Revisions a "stateless" server answers for. Ignored by the "handshake" lifecycle, which negotiates.')
                            ...
                            ->defaultValue([ProtocolVersion::FIRST_MODERN_VERSION->value])
                            ->validate()
                                ->ifTrue(static fn (array $v): bool => [] === $v)
                                ->thenInvalid('A stateless server must answer for at least one protocol revision.')
                            ->end()
                        ->end()

                        ->arrayNode('request_state')
                            ->addDefaultsIfNotSet()
                            ->info('Signs the state a multi-round-trip answer carries through the client. Required for any "stateless" server whose handlers return an InputRequiredResult.')
                            ->children()
                                ->stringNode('key')->defaultNull()->info('HMAC key. The same value must reach every process that might serve the retry.')->end()
                        ...
                        ->arrayNode('cache')
                            ->addDefaultsIfNotSet()
                            ->info('Cache hints a "stateless" server puts on its answers. The spec requires them on server/discover, the list methods and resources/read.')
                            ->children()
                        ...
                        ->arrayNode('subscriptions')
                            ...
                            ->children()
                                ...
                                ->stringNode('cache_pool')->defaultValue('cache.mcp.notifications')->info('PSR-16 service for the "cache" bus. Under PHP-FPM the publisher and the stream are different workers, so "memory" cannot reach them.')->end()
                        ...
                        ->arrayNode('tasks')
                            ->addDefaultsIfNotSet()
                            ->info('The tasks extension (SEP-2663): a durable handle instead of a held-open connection.')
                            ->children()
                                ->enumNode('store')->values(['none', 'memory', 'cache'])->defaultValue('none')->end()
                                ->stringNode('cache_pool')->defaultValue('cache.mcp.tasks')->info('PSR-16 service for the "cache" store. The worker that creates a task is not the one polled for it, so "memory" only works in a single-process runtime.')->end()
                                ->stringNode('input_handler')->defaultNull()->info('Service id implementing Mcp\Server\Task\TaskInputHandlerInterface, for answering a parked task.')->end()
                            ->end()
                        ->end()
                        ...
                        ->append($elements('tools', 'tools'))
                        ->append($elements('prompts', 'prompts'))
                        ->append($elements('resources', 'resources'))
                        ->append($elements('resource_templates', 'resource templates'))
                        ->append($elements('apps', 'MCP Apps'))
                    ->end()
                    ->validate()
                        ->ifTrue(static fn (array $v): bool => [] === $v['tools'] && [] === $v['prompts']
                            && [] === $v['resources'] && [] === $v['resource_templates'] && [] === $v['apps'])
                        ->thenInvalid('An MCP server must expose at least one of "tools", "prompts", "resources", "resource_templates" or "apps". Use ["*"] to expose every element of a kind.')
                    ->end()
                    ->validate()
                        // 2026-07-28 is HTTP-only; STDIO belongs to the handshake era.
                        ->ifTrue(static fn (array $v): bool => 'stateless' === $v['lifecycle'] && $v['transports']['stdio'])
                        ->thenInvalid('A "stateless" server cannot be served over STDIO: the 2026-07-28 lifecycle is defined over HTTP. Set "transports.stdio" to false, or use a second server for STDIO.')
                    ->end()
                    ->validate()
                        ->ifTrue(static fn (array $v): bool => 'stateless' === $v['lifecycle'] && !$v['transports']['http'])
                        ->thenInvalid('A "stateless" server needs the HTTP transport.')
                    ->end()
                ->end()
            ->end()
