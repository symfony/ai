            ...
            ->arrayNode('oauth')
                ...
                ->children()
                    ->arrayNode('authorization_servers')
                        ->scalarPrototype()->end()
                        ->defaultValue([])
                        ->info('List of OAuth 2.0 authorization server URLs (RFC 9728)')
                    ->end()
                    ->scalarNode('resource')
                        ->defaultNull()
                        ->info('Canonical URI of this MCP server. Defaults to {base_url}{http.path}')
                    ->end()
                    ->arrayNode('scopes_supported')
                        ->scalarPrototype()->end()
                        ->defaultValue([])
                        ->info('List of OAuth scopes supported by this server')
                    ->end()
                ->end()
            ->end()
        ->end()
